...
[
    ...
    2, 2,   -1, 2,   -1, -1, // triangle 1
    2, 2,   -1, -1,   2, -1, // triangle 2
...
//----------------------------------------------------------------------------
function setUpTextures(imageInput)
{
...
window.onload = function init()
{
    ...
    //Set up textures
    // ********** PARAMETER FOR SELECTING AN IMAGE **********
    setUpTextures(imageValue);
...
function render()
{
    ...
    //draw cube
    mv = mult(mv, rotateX(xrot));
    mv = mult(mv, rotateY(yrot));
    mv = mult(mv, rotateZ(zrot));
...
document.onkeydown = function handleKeyDown(event) {
   ...
   var key = String.fromCharCode(c);
    ...
    // ********** KEYBOARD INPUT CHECKS FOR IMAGE SELECTION ********** 
    if (key == "1") {
        imageValue = 1;
    }
    if (key == "2") {
